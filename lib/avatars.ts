...
export function parseAvatar(input: unknown, fallbackSeed: string): Avatar {
  ...
  const validEmoji = typeof emoji === 'string' && AVATAR_EMOJIS.includes(emoji as (typeof AVATAR_EMOJIS)[number])
  const validColor =
    typeof color === 'string' && AVATAR_COLORS.includes(color as (typeof AVATAR_COLORS)[number])
